...
function GoogleButton({ role }: { role: string }) {
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      startTransition(() => {
        signIn('google', { 
          callbackUrl: `/api/complete-google-signup?role=${role}`,
          redirect: true,
        }).catch((err) => {
  ...
  return (
    <>
      <button
        ...
        className="flex w-full items-center justify-center gap-3 rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm font-medium text-gray-700 shadow-sm transition hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
      >
      ...
      {error && (
        <div className="mt-2 flex items-center gap-2 rounded-lg bg-red-50 px-3 py-2 border border-red-100">
